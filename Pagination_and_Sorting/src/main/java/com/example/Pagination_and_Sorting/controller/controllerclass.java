...
public class controllerclass
{
    ...
    @GetMapping("mobile/{page}/{size}/{field}")
    public ResponseEntity<Page<outputdto>> getdatabypaginationandsorting(@PathVariable int page, @PathVariable int size,
                                                               @PathVariable String field)
    {
        Page<outputdto> objectofsorting=impli.getobjectbysorting(page,size,field);
        return ResponseEntity.ok()
                .body(objectofsorting);
    }
